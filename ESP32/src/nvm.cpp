...
void Nvm_ReadAll(Screen &screen)
{
   if (!EEPROM.begin(sizeof(NvmStructure_T)))
   {
      screen.log.println("Nvm readAll failed");
      while(1){}
   }
   screen.log.println("Reading Nvm");
...
void Nvm_WriteAll(Screen &screen)
{
   ...
   if (!EEPROM.commit())
   {
      screen.log.println("Nvm writeAll failed");
      while(1){}
   }
}
